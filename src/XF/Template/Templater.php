...
class Templater
{
	...
	public function handleTemplateError($errorType, $errorString, $file, $line)
	{
		...
		if ($errorType == E_WARNING && PHP_MAJOR_VERSION >= 8)
		{
	...
	public function fnAvatar($templater, &$escape, $user, $size, $canonical = false, $attributes = [])
	{
		...
		else
		{
			...
			$tag = 'span';
			$hrefAttr = ' title="' . htmlspecialchars($username) . '"';
		}
	...
	protected function getPrefixPhrase($contentType, $prefixId, $phraseType)
	{
		...
		switch ($phraseType)
		{
			...
			case 'help':
				// these allow HTML
				return \XF::phrase(sprintf('%s_prefix_%s.%d', $contentType, $phraseType, $prefixId), []);
