...
class ActivitySummary extends Repository
{
	...
	public function addInstanceSpecificDisplayValues(Instance $instance)
	{
		...
		if ($user->alerts_unviewed)
		{
			...
			if ($user->alerts_unviewed)
			{
				$instance->addDisplayValue('alerts', $user->alerts_unviewed);
			}
		}

		if ($user->conversations_unread)
		{
			$instance->addDisplayValue('conversations', $user->conversations_unread);
		}
		...
		if ($reactionScore)
		{
			$instance->addDisplayValue('reaction_score', $reactionScore);
		}
